<script lang="ts">
	...
	onMount(() => {
		fetchData('https://ptvapiwrapper.azurewebsites.net/trains/get-all-routes').then(
			(responseData) => {
				if (responseData) {
		...
		fetchData('https://ptvapiwrapper.azurewebsites.net/trains/get-all-stops').then(
			(stopsDataResponse) => {
  ...
  afterUpdate(() => {
	...
	async function fetchNextTrains() {
		try {
			if (selectedStation) {
				const departuresResponse = await fetchData(
	...
	function departureTimeWithMinutesLeft(departureTime: Date) {
		...
		const diffInMilliseconds: number =
			Number(scheduledTime.getTime()) - Number(currentTime.getTime());
		...
		<Table striped={true}>
			...
			<TableBody>
				{#each trainRoute.filter( (route) => (departuresData?.departures ?? []).some((departure) => departure.route_id === route.route_id) ) as route}
					<TableBodyRow>
						<TableBodyCell style="background-color: lightgrey; color: black;"
							>{getRouteName(route.route_id)}</TableBodyCell
						>
						{#each organizeDeparturesByRoute(route.route_id) as departure}
							<TableBodyCell style="background-color: lightyellow;">
								<div>
									<div>
										{departureTimeWithMinutesLeft(new Date(departure.scheduled_departure_utc))}
									</div>
